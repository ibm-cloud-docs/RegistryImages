...
9. (선택사항) 고급 옵션 섹션에서 볼륨을 지정하십시오. 컨테이너 마이그레이션을 통해 {{site.data.keyword.IBM_notm}} MQ 데이터와 구성을 지속적으로 저장하려면 {{site.data.keyword.Bluemix_notm}} 볼륨을 사용하십시오. 컨테이너가 종료되면 볼륨에 저장되지 않은 {{site.data.keyword.IBM_notm}} MQ 데이터는 유실됩니다.
    ...
    기본적으로 볼륨이 없이 작성된 컨테이너는 컨테이너가 중지되면 모든 IBM MQ 데이터가 유실됩니다. 이 문제점을 방지하려면 지속적 메시징과 {{site.data.keyword.Bluemix_notm}} 볼륨을 사용해야 합니다.
...
|명령 |조치 |
|---------|---------|
|dspmqver |IBM MQ 버전과 빌드 정보를 확인합니다. |
|dspmq |컨테이너에서 실행 중인 큐 관리자의 상태를 확인합니다. | 
|runmqsc |MQ 리소스를 관리합니다. |
...
|MQ_TLS_PASSPHRASE |MQ_TLS_KEYSTORE 환경 변수에 지정된 키 저장소의 비밀번호를 설정하려면 이 환경 변수를 설정하십시오. |  |
