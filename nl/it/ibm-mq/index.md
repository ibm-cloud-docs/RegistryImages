...
# Introduzione all'immagine **ibm-mq**
{: #ibm_mq}

L'immagine ibm-mq viene fornita per {{site.data.keyword.containershort}}. Include {{site.data.keyword.IBM_notm}} MQ Advanced for Developers (non coperto da garanzia). 
...
{{site.data.keyword.IBM_notm}} MQ Advanced for Developers contiene tutto ciò che ti serve per iniziare a sviluppare le tue proprie soluzioni di messaggistica e applicazioni {{site.data.keyword.IBM_notm}} MQ. 
...
{{site.data.keyword.IBM_notm}} MQ Advanced for Developers è disponibile per i sistemi operativi Windows a 64 bit e Linux su x86-64. Tutti i prerequisiti del prodotto sono inclusi nel pacchetto di download. Per ulteriori informazioni, vedi [IBM MQ Downloads ![Icona link esterno](../../../icons/launch-glyph.svg "Icona link esterno")](http://www-03.ibm.com/software/products/lv/ibm-mq-advanced-for-developers){: new_window}

Per informazioni sulle funzioni incluse in {{site.data.keyword.IBM_notm}} MQ Advanced for Developers, vedi [IBM MQ ![Icona link esterno](../../../icons/launch-glyph.svg "Icona link esterno")](https://www.ibm.com/support/knowledgecenter/SSFKSJ_9.0.0/com.ibm.mq.helphome.v90.doc/WelcomePagev9r0.htm){: new_window}
...
1. [Esegui il provisioning di un contenitore basato sull'immagine ibm-mq](#ibm_mq_provision)
...
2. Gestisci le risorse {{site.data.keyword.IBM_notm}} MQ in esecuzione in un contenitore. Puoi farlo graficamente dalla console web di IBM MQ o a livello di programmazione usando i comandi da un terminale. 
    ...
    1. Scegli una delle seguenti opzioni per gestire le risorse di {{site.data.keyword.IBM_notm}} MQ nel contenitore:
        ...
        *	Avvia la console web di {{site.data.keyword.IBM_notm}} MQ e gestisci {{site.data.keyword.IBM_notm}} MQ graficamente. Per ulteriori informazioni, vedi [Avvia la console web di IBM MQ](#ibm_mq_webui).
        *	Da un terminale, utilizza la CLI Docker. Per ulteriori informazioni, vedi [Configurazione di un terminale per l'utilizzo della CLI Docker](#ibm_mq_docker_commands).
        * Da un terminale, utilizza la CLI {{site.data.keyword.containershort_notm}}. Per ulteriori informazioni, vedi [Configurazione di un terminale per l'utilizzo della CLI {{site.data.keyword.containershort_notm}}](#ibm_mq_containers_cli)
        
    2. Configura il gestore code {{site.data.keyword.IBM_notm}} MQ che viene eseguito nel contenitore. 

        * Per gestire il gestore code attraverso la riga di comando, utilizza il programma *runmqsc*. Esegui questo comando: `runmqsc QM_Name` dove *QM_Name* è il nome del tuo gestore code. Per ulteriori informazioni, vedi [Gestione del tuo contenitore attraverso la riga di comando](#ibm_mq_access).
...
3. (Facoltativo) Monitora i log {{site.data.keyword.IBM_notm}} MQ nel contenitore. Per ulteriori informazioni, vedi [Gestione del tuo contenitore attraverso la riga di comando](#ibm_mq_access).
...
Per ulteriori informazioni sull'immagine e su come creare la tua propria immagine localmente in Docker, vedi [la pagina MQ Docker GitHub ![Icona link esterno](../../../icons/launch-glyph.svg "Icona link esterno")](https://github.com/ibm-messaging/mq-docker){: new_window}
...
10.	Fai clic su **Crea** e attendi che il contenitore avvii l'esecuzione.
    ...
    Per controllare i log del tuo contenitore, fai clic su **Monitoraggio e log** e seleziona la scheda **Registrazione**. Verranno visualizzati i log. Per un'analisi avanzata dei tuoi log, fai clic su **Vista avanzata** per visualizzare i tuoi log in Kibana.
...
1.	Configura la CLI {{site.data.keyword.containershort_notm}}. Per ulteriori informazioni, consulta [la pagina CLI {{site.data.keyword.Bluemix_notm}}](https://plugins.ng.bluemix.net/ui/home.html) e segui le istruzioni per installare il plug-in {{site.data.keyword.containershort_notm}} più recente.
...
3.	Controlla lo stato del contenitore. Immetti il seguente comando: `bx ic ps`
...
* Se il tuo terminale è configurato per eseguire i comandi della CLI {{site.data.keyword.containershort_notm}}, esegui questo comando: `bx ic exec container_id tail -f /var/mqm/qmgr/QM_ Name/errors/AMQERR01.LOG` dove *container_id* è il nome del tuo contenitore e *QM_ Name* è il nome del tuo gestore code.
...
### Configurazione di un terminale per l'utilizzo della CLI {{site.data.keyword.containershort_notm}}
{: #ibm_mq_containers_cli}

Utilizza la CLI {{site.data.keyword.containershort_notm}} per eseguire i comandi di amministrazione di {{site.data.keyword.IBM_notm}} MQ direttamente in un contenitore.

Completa la seguente procedura per configurare un terminale per l'esecuzione dei comandi `bx ic` per gestire IBM MQ:
...
2.	Da un terminale, accedi a {{site.data.keyword.Bluemix_notm}}. Esegui questi comandi:
    ...
    4. Sovrascrivi l'ambiente Docker locale impostando le seguenti variabili per il collegamento a {{site.data.keyword.containershort_notm}}:
...
| Variabile di ambiente | Scopo | Impostazione predefinita |
|----------------------|---------|---------|
